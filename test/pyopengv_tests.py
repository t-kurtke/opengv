...
class NoncentralAbsolutePoseDataset:
    def __init__(self, num_points, num_cams, noise, outlier_fraction=None):
        self.num_cams = num_cams
        self.cam_offsets = []
        self.cam_rots = []
        for cam_index in range(num_cams):
            self.cam_offsets.append(generateRandomPoint(-2, 2))
            self.cam_rots.append(generateRandomRotation(1))

        self.viewpoint_offset = generateRandomPoint(-2, 2)
        self.viewpoint_rot = generateRandomRotation(1)
        for cam_index in range(num_cams):
            self.cam_offsets.append(generateRandomPoint(-2, 2))
            self.cam_rots.append(generateRandomRotation(1))
        self.correspondences = self.generate_correspondences(num_points, noise, outlier_fraction)
    def generate_correspondences(self, num_points, noise, outlier_fraction,):
        ...
        for i in range(num_points):
            for j in range(self.num_cams):
                cam_pos_world = self.viewpoint_offset + self.viewpoint_rot.dot(self.cam_offsets[j])
                cam_rot_world = self.viewpoint_rot * self.cam_rots[j]
                body_point = cam_rot_world.T.dot(self.points[i] - cam_pos_world)
                bearing_vector = normalized(body_point)
                # add noise
                if noise > 0.0:
                    bearing_vector = addNoise(noise, bearing_vector)

                self.bearing_vectors.append(bearing_vector)
                self.cam_correspondences.append(j)
        return np.array(self.bearing_vectors), np.array(self.cam_correspondences), np.array(self.points), np.array(self.cam_offsets), np.array(self.cam_rots)


def test_noncentral_absolute_pose():
    print("Testing noncentral absolute pose")
    d = NoncentralAbsolutePoseDataset(10, 2, 0.0)
    gp3p_pose = pyopengv.absolute_pose_noncentral_gp3p(*d.correspondences)
